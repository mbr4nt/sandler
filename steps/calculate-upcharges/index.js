...
module.exports = async function calculateUpcharges(collection, range, product, readFile, writeFile) {
    console.log(`Calculating upcharges for ${collection}/${range}/${product}`);
    ...
    pricesStream
        ...
        .on('end', async () => {
            ...
            for (const priceInfo of prices) {
                ...
                if (deficit > 0) {
                    ...
                    if (nonCheapestOptions.length === 0) {
                        console.error(`Configuration ${configCombination} requires upcharge ${requiredUpcharge} but has no non-cheapest options.`);
                        continue;
                    }
            ...
            console.log(`Upcharges calculated and written to ${upchargesFilePath}`);
        });
};